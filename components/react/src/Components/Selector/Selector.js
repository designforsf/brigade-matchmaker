...
const Selector = ({ categories, name, taxonomyId, addToMediator }) => {
  ...
  const selectorCategory = (categoryName, id) => (
    <Category
      categoryId={id}
      name={categoryName}
      click={() => removeCategoryFromSelected(id)}
      key={`category${id}`}
    />
  );
  const selectorTag = (tagName, id) => (
    <Tag
      tagId={id}
      name={tagName}
      click={() => removeTagFromSelected(id)}
      key={`tag${id}`}
    />
  );
  ...
  const addToSelected = (type, name, id) => {
    const tagToAdd =
      type === "tag" ? selectorTag(name, id) : selectorCategory(name, id);
    setSelected(
      _.uniqBy([tagToAdd, ...selected], selectedItem =>
        JSON.stringify(getTagId(selectedItem))
      )
    );
  };
  ...
  const removeCategoryFromSelected = id => {
    setSelected(selected =>
      selected.filter(category => category.props.categoryId !== id)
    );
  };
  ...
  return (
    <div className="card">
      <div className="card-header text-center">
        <div className="selector-box">
          ...
          <button
            data-toggle="modal"
            className="add-items"
            data-target={`#selector-modal-${taxonomyId}`}
          >
            Add Item
          </button>
        </div>
      ...
      <div className="card-body">
        {selected}
        <SelectorModal
          categories={categories.map(modalCategory)}
          name={name}
          key={taxonomyId}
